...
/**
 * \file    sav/admin/setup.php
 * \ingroup sav
 * \brief   Sav setup page.
 */
...
require_once '../lib/sav.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "sav@sav"));
...
$arrayofparameters = array(
	'SAV_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'SAV_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/sav/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'SAV_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'SAV_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_sav@sav');

// Configuration header
$head = savAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "sav@sav");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("SavSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'SAV_'.strtoupper($myTmpObjectKey).'_ADDON';
